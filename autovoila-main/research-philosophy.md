...
### Applying the philosophy in an autonomous research workflow

When research is conducted with an AI research agent, the same principles apply. The agent should optimize for discovering useful knowledge rather than simply completing experiments or producing a paper.

The human researcher should provide direction and make major research choices, while the agent handles exploration, literature review, experimentation and synthesis autonomously where appropriate.

During **exploration**, consider multiple possible claims before committing to one. Do not become attached to the first plausible hypothesis.

During **research question sharpening**, explicitly consider whether the claim is already known, whether it matters if true, what evidence would distinguish it from alternative explanations, and whether it is feasible within the available resources and time.

### Additional Instructions

During **experiment execution**, prioritize experiments that provide the most information about the research question. Experiments should be designed not only to support a hypothesis, but also to distinguish it from plausible alternatives.

An independent critic, such as Codex, can be used at important decision points to challenge assumptions, identify alternative explanations and suggest stronger experiments. Agreement from the critic should not be treated as evidence.

The research should remain time-bounded. When resources are limited, prioritize the experiments and analyses that are most likely to change our understanding of the research question.

It is acceptable for the research to conclude that the original hypothesis is unsupported, the research question is not sufficiently interesting, or that the available evidence is inconclusive. The goal is to discover what is true and interesting, not to produce a positive result.